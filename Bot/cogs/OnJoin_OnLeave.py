...
class OnJoin_OnLeave(commands.Cog):
    ...
    async def on_member_join(self, member : commands.MemberConverter):
        ...
        await channel.send(f"{member.mention} joined the server. We now have {member.guild.member_count} members")

    @commands.Cog.listener() 
    async def on_member_remove(self, member):
        channel = member.guild.get_channel(908977348353880095)
        await channel.send(f'{member.mention} left the server. We now have {member.guild.member_count} members')
